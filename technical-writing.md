...
프론트엔드 영역에서 상태란, 웹 애플리케이션을 구성하는 **데이터**입니다. 무조건 데이터라고 해서 상태가 될 수 있는 것은 아닙니다. 상태인 것과 그렇지 않은 것을 구분하는 가장 큰 조건은 변할 수 있는 데이터여야 한다는 것입니다. 상태는 **변할 수 있는 데이터**입니다.
...
값을 입력할 때마다, 입력값 데이터가 변경되며 입력 필드에 입력한 값이 UI로 표현됩니다.
...
하나의 데이터에 대해서 여러 UI를 표현할 때, **잘못된 상태 관리**로 인해서 같은 데이터를 표현하지 못하면 일관되지 못한 UI로 인해서 **사용자에게 좋지 못한 경험을 제공**할 수 있습니다. 위 이미지를 예로 들자면 사용자에게 보여줘야 하는 데이터가 하나 있고 여러 UI를 표현하고 있습니다. 데이터는 1234인데 UI는 12만 표현한다면 사용자는 혼란을 느끼게 됩니다. 그리고, 서비스의 규모가 커지고 상태의 복잡도가 증가할 때 상태를 효율적으로 관리하지 않는다면 코드가 자연스럽게 복잡해질 수 있습니다. 협업 관점에서도 어떻게 상태를 관리할 것인지 명확하게 정해야 합니다.
...
UI의 스냅샷이라는 것은 컴포넌트를 렌더링하는 시점의 props와 상태를 기반으로 UI를 생성하기 때문입니다. 이는 마치, 사진을 찍어서 컴포넌트의 순간적인 모습을 담아내는 것과 같기 때문에 스냅샷이라는 표현을 사용합니다.
...
- **리액트에서의 상태란, 컴포넌트의 렌더링 결과물**에 영향을 미치는 **동적인 데이터**이다.
- 상태가 변할 때마다, **UI를 다시 그려줄 것을 요청**하며 이 과정을 렌더링이라고 합니다.
- 리액트에서 컴포넌트는 DOM 트리 구조처럼, 컴포넌트 트리 구조를 가집니다.
- 상태가 변경되었을 때, 최상위 노드부터 **변경이 필요한 컴포넌트를 찾아 렌더링**한다.
- 부모 컴포넌트가 렌더링되면 **일반적으로 모든 하위 자식 컴포넌트들도 재귀적으로 렌더링**된다.
...
위에서 상태는 컴포넌트의 기억 저장소라고 했었습니다. 리액트에서는 [자바스크립트 클로저](https://developer.mozilla.org/ko/docs/Web/JavaScript/Closures) 개념을 활용해서 상태를 기억할 수 있도록 합니다. 정확하지는 않지만 간단하게 useState를 직접 구현해 보며 어떻게 상태를 기억할 수 있는 것인지 알아봅시다. 이 과정은 개념을 이해하기 위한 것이므로, **실제 리액트의 내부 구현과는 차이가 있을 수 있습니다.**
...
버튼 컴포넌트에서는 상태를 변경하는 컨텍스트만 참조하도록 하면 불필요한 렌더링은 막을 수 있습니다. 하지만 번 이렇게 두 개의 프로바이더를 사용한다면 코드의 복잡성을 야기하고 가독성을 해칠 수 있습니다.
